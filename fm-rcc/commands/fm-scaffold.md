---
description: Scaffold a FileMaker project folder (minimized by default; --full or --client-kit for more)
argument-hint: "[--full] [--client-kit]"
allowed-tools: Bash, Read, Write, Glob
---

Scaffold the standard project structure into the current directory from the plugin template. **Never overwrite an existing file** — skip it and note the skip in the report.
...
3. **`--client-kit`** (in $ARGUMENTS) — additionally create the per-client overlay skeleton at `client-kit/` (the deliverable that ships to the client, per the client-kit overlay model):
...
4. **Fill what's knowable:** set the folder name into `CLAUDE.md`'s `<Project Name>` and `fm-rcc.json`-less defaults; leave client-specific blanks alone.

5. **Report** the tree created, files skipped, and next steps: fill `_pm/skeleton.md`, copy `.env.example` → `.env`, then `/fm-rcc:fm-init` to adopt the .fmp12.
